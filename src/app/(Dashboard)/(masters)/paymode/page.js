"use client";
import { Box, Typography, List, ListItemButton, FormControl, Select, InputLabel, MenuItem, Menu, Button, IconButton, Drawer, TextField, Divider, Badge, } from "@mui/material";
...
const EmployeeList = () => {
  ...
  const handleClickEdit = () => {
    ...
    setOpenDrawer(true);
    handleClose2();
    setPayMode(singleData.paymode_type);
  ...
  const handleSubmit = () => {
    ...
    const jsonData = {
      ...
      paymode_type: payMode,
      active_status: dataStatus,
    };
    try {
      if (openDrawerType == 1) {
        axiosPost
          .post(`paymode_master`, jsonData)
          .then((response) => {
  ...
  const handleDelete = () => {
    ...
    axiosPost
      .delete(`paymode_master`, { data: jsonData })
      .then((response) => {
  ...
  const handleStatusChange = () => {
    ...
    axiosPost
      .post(`paymode_master_status`, jsonData)
      .then((response) => {
        setEffectToggle(!effectToggle);
        setSelectedItems([]);
        setActionData("");
  ...
  const handleMulitiDelete = () => {
    ...
    axiosPost
      .post(`paymode_master_delete`, jsonData)
      .then((response) => {
        setEffectToggle(!effectToggle);
        setSelectedItems([]);
        setActionData("");
  ...
  const fetchData = async () => {
    ...
    axiosGet
      .get(
        `paymode_master_get?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${orderType}&order_field=${orderField}&active_status=${activeStatusFilter === 3 ? "" : activeStatusFilter
        }`
      )
      .then((response) => {
  ...
  const userHasActionPrivilege =
    privileges.includes("editPaymode") || privileges.includes("deletePaymode");
  ...
  // if (!privileges.includes("viewPaymode")) {
  ...
  return (
    <div style={{ padding: "10px" }}>
      ...
      >
        <CreateButton
          allowed={privileges.includes("createPaymode")}
          heading={title}
          pagecount={dataCount}
          ...
          <Box sx={{ p: 1 }}>
            <Box sx={{ my: 1 }}>
              <TextField
                ...
                helperText={postError?.paymode_type}
                error={postError?.paymode_type}
              />
            </Box>
          </Box>
        </Box>
      ...
      >
        ...
        <List sx={{ p: 0, fontSize: "14px" }}>
          {privileges.includes("editPaymode") && (
            <>
          ...
          {privileges.includes("deletePaymode") && (
            <ListItemButton sx={{ '&:hover': { color: 'red', } }} onClick={handleDelteOpen} >
