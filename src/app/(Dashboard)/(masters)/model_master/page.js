...
const EmployeeList = () => {
  ...
  const handleTagChange = (e) => {
    ...
    if (/^[A-Za-z]{0,2}$/.test(value)) {
      if (value.length === 2) {
        ...
        setTagError("Team Tag must be exactly 2 letters");
        setTeamTag(value);
      }
    } else {
      setTagError("Team Tag must be exactly 2 letters");
  ...
  const handleDelete = () => {
    ...
    axiosPost
      .delete(`model_master`, { data: jsonData })
      .then((response) => {
  ...
  const handleMulitiDelete = () => {
    ...
    axiosPost
      .post(`model_master_delete`, jsonData)
      .then((response) => {
        setEffectToggle(!effectToggle);
        setSelectedItems([]);
        setActionData("");
  ...
  const handleStatusChange = () => {
    ...
    axiosPost
      .post(`model_master_status`, jsonData)
      .then((response) => {
        setEffectToggle(!effectToggle);
        setSelectedItems([]);
        setActionData("");
  ...
  const fetchData = async () => {
    ...
    axiosGet
      .get(
        `model_master_get?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${orderType}&order_field=${orderField}&active_status=${
  ...
  const title = "Model";
  // const userHasActionPrivilege =    privileges.includes("editTeam") || privileges.includes("deleteTeam");
  ...
  // if (!privileges.includes("viewTeam")) {
          ...
          <Box sx={{ p: 1 }}>
            <Box sx={{ my: 1 }}>
              <TextField
                ...
                helperText={postError?.model}
                error={postError?.model}
              />
            </Box>
          </Box>
        </Box>
