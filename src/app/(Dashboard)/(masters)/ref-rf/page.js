...
const EmployeeList = () => {
  ...
  const handleDelete = () => {
    ...
    axiosPost
      .delete(`rfid_master`, { data: jsonData })
      .then((response) => {
  ...
  const handleStatusChange = () => {
    ...
    axiosPost
      .post(`rfid_master_status`, jsonData)
      .then((response) => {
        setEffectToggle(!effectToggle);
        setSelectedItems([]);
        setActionData("");
  ...
  const handleMulitiDelete = () => {
    ...
    axiosPost
      .post(`rfid_master_delete`, jsonData)
      .then((response) => {
        setEffectToggle(!effectToggle);
        setSelectedItems([]);
        setActionData("");
  ...
  const fetchData = async () => {
    ...
    axiosGet
      .get(
        `rfid_master_get?access_token=${ACCESS_TOKEN}&page=${pageNumber}&items_per_page=${limitEnd}&search_input=${searchValue}&from_date=${createdStartDate}&to_date=${createdEndDate}&order_type=${orderType}&order_field=${orderField}&active_status=${activeStatusFilter === 3 ? "" : activeStatusFilter
        }`
      )
      .then((response) => {
  ...
  useEffect(() => {
    fetchData();
  }, [ACCESS_TOKEN, pageNumber, limitEnd, searchValue, createdStartDate, createdEndDate, orderField, orderType, openDrawer, effectToggle, activeStatusFilter,]);

  const title = "Ref Rf Id";
          ...
          <Box sx={{ p: 1 }}>
            <Box sx={{ my: 1 }}>
              <TextField
                ...
                helperText={postError?.rf_id}
                error={postError?.rf_id}
              />
            </Box>
          </Box>
        </Box>
